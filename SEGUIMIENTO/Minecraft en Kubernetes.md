Ejemplo listo para copiar de: "La monitorización de un servidor de Minecraft desplegado en Kubernetes combina dos cosas: métricas del clúster + métricas específicas del servidor de juego. Te explico cómo montarlo de forma práctica y escalable. 🧱 1. Arquitectura básica Un setup típico incluye: Kubernetes → Orquesta tu servidor de Minecraft Prometheus → Recolecta métricas Grafana → Visualiza dashboards Exportadores específicos: Métricas del pod (CPU, RAM, red) Métricas de Minecraft (TPS, jugadores, ticks, etc.)"

Perfecto — aquí tienes un **ejemplo completo, mínimo y listo para copiar** para monitorizar un servidor de Minecraft en Kubernetes (con **k3s + Prometheus + Grafana + exporter**). Está pensado para tu portátil, así que es **ligero y funcional**.
...
# 🧠 10. Versión ULTRA ligera (para tu portátil)
